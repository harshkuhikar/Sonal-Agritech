...
// My Custom Slider Start
document.addEventListener("DOMContentLoaded", function () {
  const customSliderTrack = document.getElementById("customSliderTrack");
  const customSlides = document.querySelectorAll(".customSlider-slide");
  const btnPrev = document.querySelector(".customSlider-prev");
  const btnNext = document.querySelector(".customSlider-next");
  ...
  function updateSliderPosition() {
    customSliderTrack.style.transform = `translateX(-${
      customCurrentIndex * (100 / slidesPerView)
    }%)`;
  }
  ...
  btnNext.addEventListener("click", () => {
    ...
    if (customCurrentIndex < maxIndex) {
      customCurrentIndex++;
      updateSliderPosition();
    }
  });
...
// My Custom Slider End

document.addEventListener("DOMContentLoaded", function () {
  const mylogo = document.getElementById("logoLink");
  const audio = document.getElementById("logoSound");

  mylogo.addEventListener("click", function (event) {
    event.preventDefault();
    audio.currentTime = 0;
    audio.play();
  });
});
const mainFab = document.getElementById("advanced-main-fab");
const fabOptions = document.getElementById("advanced-fab-options");

mainFab.addEventListener("click", () => {
  const isActive = fabOptions.classList.toggle("active");
  mainFab.classList.toggle("open", isActive);
});

// Allow keyboard navigation for accessibility
mainFab.addEventListener("keydown", (e) => {
  if (e.key === "Enter" || e.key === " ") mainFab.click();
});
